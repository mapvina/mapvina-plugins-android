...
import io.github.mapvina.android.plugins.offline.OfflinePluginConstants;
import io.github.mapvina.android.plugins.offline.R;
import io.github.mapvina.android.plugins.offline.model.RegionSelectionOptions;
...
public class RegionSelectionFragment extends Fragment implements OnMapReadyCallback,
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.mapvina_offline_region_selection_fragment, container, false);
        mapView = rootView.findViewById(R.id.mapvina_offline_region_selection_map_view);
        regionNameTextView = rootView.findViewById(R.id.mapvina_offline_region_name_text_view);
    ...
    @Override
    public void onMapReady(final MapVinaMap mapvinaMap) {
        this.mapVinaMap = mapvinaMap;
        mapvinaMap.setStyle(Style.getPredefinedStyle("Streets"), new Style.OnStyleLoaded() {
            @Override
            public void onStyleLoaded(@NonNull Style style) {
                RegionSelectionFragment.this.style = style;
                mapvinaMap.addOnCameraIdleListener(RegionSelectionFragment.this);
                if (options != null) {
                    if (options.startingBounds() != null) {
                        mapvinaMap.moveCamera(CameraUpdateFactory.newLatLngBounds(options.startingBounds(), 0));
                    } else if (options.statingCameraPosition() != null) {
                        mapvinaMap.moveCamera(CameraUpdateFactory.newCameraPosition(options.statingCameraPosition()));
                    }
                }
            }
        });
    ...
    private RectF getSelectionRegion() {
        View selectionBoxView = rootView.findViewById(R.id.mapvina_offline_scrim_view);
        int paddingInPixels = (int) getResources().getDimension(R.dimen.mapvina_offline_scrim_padding);
    ...
    public String getOfflineRegionName() {
        List<Feature> featureList = mapVinaMap.queryRenderedFeatures(boundingBox, LAYER_IDS);
        ...
        return getString(R.string.mapvina_offline_default_region_name);
    }

    OfflineRegionDefinition createRegion() {
        if (mapVinaMap == null) {
            throw new NullPointerException("mapvinaMap is null and can't be used to create Offline region"
                + "definition.");
        }
        RectF rectF = getSelectionRegion();
        LatLng northEast = mapVinaMap.getProjection().fromScreenLocation(new PointF(rectF.right, rectF.top));
        LatLng southWest = mapVinaMap.getProjection().fromScreenLocation(new PointF(rectF.left, rectF.bottom));

        LatLngBounds bounds = new LatLngBounds.Builder().include(northEast).include(southWest).build();
        double cameraZoom = mapVinaMap.getCameraPosition().zoom;
        float pixelRatio = getActivity().getResources().getDisplayMetrics().density;

        return new OfflineTilePyramidRegionDefinition(
            mapVinaMap.getStyle().getUrl(), bounds, cameraZoom - 2, cameraZoom + 2, pixelRatio
        );
    }

    private void bindClickListeners() {
        FloatingActionButton button = rootView.findViewById(R.id.mapvina_offline_select_region_button);
