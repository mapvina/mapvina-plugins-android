...
function createMap(t) {
    config.ACCESS_TOKEN = 'pk.REMOVED';
    ...
    map.on('load', () => {
        ...
        map.on('data', (e) => {
            if (e.dataType === 'source' && e.sourceDataType === 'metadata') {
                ...
                t.equal(attribution._editLink.href, 'https://feedback.com/?owner=mapbox&id=streets-v10&access_token=pk.REMOVED#/0/0/0', 'edit link contains map location data');
                map.setZoom(2);
                t.equal(attribution._editLink.href, 'https://feedback.com/?owner=mapbox&id=streets-v10&access_token=pk.REMOVED#/0/0/2', 'edit link updates on mapmove');
                t.end();
            }
        });
    });
