...
import io.github.mapvina.android.gestures.MoveDistancesObject;
...
final class DraggableAnnotationController {
    ...
    public static DraggableAnnotationController getInstance(MapView mapView, MapVinaMap mapvinaMap) {
        if (INSTANCE == null || INSTANCE.mapView != mapView || INSTANCE.mapvinaMap != mapvinaMap) {
            INSTANCE = new DraggableAnnotationController(mapView, mapvinaMap);
        }
        return INSTANCE;
    }
    ...
    DraggableAnnotationController(MapView mapView, MapVinaMap mapvinaMap) {
        this(mapView, mapvinaMap, new AndroidGesturesManager(mapView.getContext(), false),
            mapView.getScrollX(), mapView.getScrollY(), mapView.getMeasuredWidth(), mapView.getMeasuredHeight());
    }

    @VisibleForTesting
    public DraggableAnnotationController(MapView mapView, MapVinaMap mapvinaMap,
    ...
    boolean onMove(MoveGestureDetector detector) {
        ...
        if (draggedAnnotation != null) {
            ...
            Geometry shiftedGeometry = draggedAnnotation.getOffsetGeometry(
                mapvinaMap.getProjection(), moveObject, touchAreaShiftX, touchAreaShiftY
            );
